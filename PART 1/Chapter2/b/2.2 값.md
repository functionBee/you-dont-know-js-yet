...
JS에서 문자열, 숫자, 불리언 외의 원시 값은 `null`과 `undefined`로, 모두 비어있는 값을 나타내지만, 역사적, 현대적 차이가 있습니다. null은 의도적으로 할당되지 않은 값을 나타내는 반면, undefined는 값이 할당되지 않은 경우, 함수가 값을 반환하지 않거나, 객체의 속성이 존재하지 않을 때 반환됩니다.

#### 역사적 차이

- **null:** 1995년 JavaScript 1.1에서 도입되었으며, 의도적으로 할당되지 않은 값을 나타냅니다. 변수가 선언되었지만 아무런 값도 할당되지 않은 경우 null이 됩니다.
- **undefined:** JavaScript 1.0에서부터 존재했으며, 변수가 선언되었지만 값이 할당되지 않은 경우 undefined가 됩니다. 또한, 함수가 값을 반환하지 않거나, 객체의 속성이 존재하지 않을 때도 undefined가 반환됩니다.

#### 현대적 차이

- **null:** 의도적으로 할당되지 않은 값을 나타냅니다. 변수가 선언되었지만 아무런 값도 할당되지 않은 경우 null이 됩니다.
- **undefined:** 변수가 선언되었지만 값이 할당되지 않은 경우, 함수가 값을 반환하지 않거나, 객체의 속성이 존재하지 않을 때도 undefined가 반환됩니다.
...
- null과 undefined는 모두 falsy 값입니다. 즉, 조건문에서 false로 취급됩니다.
- null과 undefined는 같지 않습니다. `null == undefined`는 true지만, `null === undefined`는 false입니다.
- ES6부터는 null과 undefined를 구분하는 `Object.is()` 메서드가 도입되었습니다.
...
많은 개발자들은 값이 구별할 수 없다고 가정하며 일관되게 취급하길 선호합니다. 하지만, 더 안전하고 좋은 방법은 'null'이 타이핑하기 더 짧아 보이더라도, 'undefined'만을 비어 있는 값으로 사용하는 것입니다!
...
인지하고 있어야 할 마지막 원시 값은 심볼인데, 이는 특수 목적의 값으로, 숨겨진 예측 불가능한 값처럼 작동합니다. 심볼은 거의 독점적으로 객체의 특수 키로 사용됩니다:
...
일반적인 JS 프로그램에서는 심볼의 직접적인 사용을 자주 접하지 않습니다. 그들은 주로 라이브러리와 프레임워크와 같은 저수준 코드에서 사용됩니다.
